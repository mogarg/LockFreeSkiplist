...
public class R2SkipList<T> {
  private static final int MAX_THREAD_COUNT = 128;
  private static final int MASK_LENGTH = 7;
  private static final int MARKED_VALUE = (Integer.MAX_VALUE >> 7);

  private final int MAX_LEVEL;
  private final Node<T> head;
  private final Node<T> tail;
  private ThreadLocalRandom random;
  private SkipListSnapshot<T>[] threadSnapshots;

  @SuppressWarnings("unchecked")
  public R2SkipList(int maxLevel) {
    ...
    threadSnapshots = (SkipListSnapshot<T>[]) new SkipListSnapshot[MAX_THREAD_COUNT];
    for (int i = 0; i < MAX_THREAD_COUNT; i++) {
      threadSnapshots[i] = null;
    }
  }
  ...
  public boolean add(T x) {
    ...
    while (true) {
      ...
      if (found) {
        ...
        threadSnapshots[getThreadId()] = collect(); // update the last snapshot
      ...
      if (!found) {
        ...
        while (true) {
          ...
          if (iMarkedIt) {
            ...
            threadSnapshots[getThreadId()] = collect(); // update the snapshot
            find(x, preds, succs);
            return true;
  ...
  public SkipListSnapshot<T> takeSnapshot() {
    ...
    while (true) {
      ...
      if (next.equals(first)) {
        ...
        threadSnapshots[getThreadId()] = next;
        return next;
      } else {
        // see if a new snapshot has been registered
        ArrayList<Integer> movedIds = getMovedThreads(first, next);

        for (Integer id : movedIds) {
          if (moved.contains(id)) {
            if (threadSnapshots[id] != null) {
              return threadSnapshots[id];
            }
          } else {
            moved.add(id);
          }
  ...
  private int getThreadId() {
    return (int) (Thread.currentThread().getId() % MAX_THREAD_COUNT);
  }
  ...
  private int getNextStamp(int oldStamp) {
    int oldStampValue = (oldStamp >> MASK_LENGTH);
    int newStampValue = oldStampValue + 1;

    if (newStampValue == MARKED_VALUE) {
      return getFirstStamp();
    } else {
      return (newStampValue << MASK_LENGTH) | getThreadId();
    }
  }

  private int getMarkedStamp() {
    return ((MARKED_VALUE << MASK_LENGTH) | getThreadId());
  }

  private boolean stampIsMarked(int stamp) {
    return (stamp >> MASK_LENGTH) == MARKED_VALUE;
  }

  private int getResponsibleThread(int stamp) {
    return (stamp & (MAX_THREAD_COUNT - 1));
  }

  private ArrayList<Integer> getMovedThreads(SkipListSnapshot<T> original, SkipListSnapshot<T> second) {
    ArrayList<Integer> moved = new ArrayList<>();

    List<Integer> originalKeys = original.getKeys();
    List<Integer> originalStamps = original.getStamps();

    List<Integer> secondKeys = second.getKeys();
    List<Integer> secondStamps = second.getStamps();

    int originalIndex = 0, secondIndex = 0;
    while (originalIndex < originalKeys.size() && secondIndex < secondKeys.size()) {
      if (!originalKeys.get(originalIndex).equals(secondKeys.get(secondIndex))) {
        if (originalKeys.get(originalIndex).compareTo(secondKeys.get(secondIndex)) < 0) {
          // the second snapshot had a missing element
          moved.add(getResponsibleThread(secondStamps.get(secondIndex)));
          originalIndex++;
          continue;
        } else {
          // the second snapshot had a new element
          moved.add(getResponsibleThread(secondStamps.get(secondIndex)));
          secondIndex++;
          continue;
        }
      } else if (!originalStamps.get(originalIndex).equals(secondStamps.get(secondIndex))) {
    ...
    while (secondIndex < secondKeys.size()) {
      moved.add(getResponsibleThread(secondStamps.get(secondIndex)));
      secondIndex++;
    }
